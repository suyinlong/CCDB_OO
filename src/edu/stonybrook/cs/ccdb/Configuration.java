...
public class Configuration {
	...
	public static String QUERY[] = {
			...
			"WITH RECURSIVE indirect_user(id, name, number) AS ( SELECT aa.id, aa.name, a.number FROM \"Account\" a, \"getNonOwnerAuthUsers\"(a) aa UNION SELECT aa.id, aa.name, iu.number FROM indirect_user iu, \"Person\" p, \"Account\" a, \"getNonOwnerAuthUsers\"(a) aa WHERE iu.id = p.id AND a.owner = p.id) SELECT * FROM indirect_user;",
			"WITH RECURSIVE indirect_user(id, name, number) AS ( SELECT aa.id, aa.name, a.number FROM \"Account\" a, \"getNonOwnerAuthUsers\"(a) aa UNION SELECT aa.id, aa.name, iu.number FROM indirect_user iu, \"Person\" p, \"Account\" a, \"getNonOwnerAuthUsers\"(a) aa WHERE iu.id = p.id AND a.owner = p.id) SELECT sum(a.balance) FROM indirect_user iu, \"Account\" a WHERE iu.name = 'Joe' AND iu.number = a.number;"
	};
